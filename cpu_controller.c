...
static uint32_t get_selection(void);
static void readline(char* s,
                     const int size);
static inline uint32_t get_byte(void);
...
static inline void print_information_at_start(void)
{
    printf("A led is connected to pin 8 (PORTA8).\n");
    printf("Press the button connected to pin 13 (PORTA13) to toggle the led.\n");
...
static int execute_selection(void)
{
    ...
    {
        printf("Enter new data for pin input register PINA:\n");
        const uint32_t input = get_byte();
...
static uint8_t get_selection(void)
{
    while (1)
    {
        const uint32_t selection = get_byte();
